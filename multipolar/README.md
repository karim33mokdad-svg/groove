...
**One file.** `node build/inline.js` inlines the stylesheet and all eleven
scripts into `dist/multipolar.html` — a single 225 KB page with no external
references at all. Open it straight from disk (`file://` works, no server
needed), email it, or drop it on any static host. `dist/multipolar.frag.html`
is the same page without the document shell, for hosts that supply their own.

**A local server**, for developing against the unbundled sources:
...
**GitHub Pages.** Serve the repository and open `multipolar/`. The service
worker caches everything on first load, so it works offline afterwards and can
...
**Pick a seat.** Each capital opens with a dossier: capability, doctrine,
declared red lines, and the five objectives you will be scored against. The
objectives are the game — read them before your first move.

**You get political capital each quarter**, roughly 4–8, generated by your
approval, legitimacy and alliance cohesion. Every instrument costs some. You
cannot do everything, and a weak position at home shrinks what you can afford
abroad. That constraint is the whole design.

**Open a theatre** to see its escalation rung, the balance between the two
sides, what is actually driving it, and the instruments available there.
**Instruments** at the bottom covers actions aimed at another capital rather
than a place.

**Read the forecast before committing.** Projected effects come with ranges;
underneath them is why the model says so, the ways it goes wrong with their
probabilities, and any red-line exposure. If the reasoning looks wrong, it is
on screen and you can disagree with it.

**Simulate first if the decision is expensive.** "Simulate this decision"
runs the same world hundreds of times with and without your move and tells you
how often it actually leaves you better off. "Run outcome projection" on the
board does the same for the board as a whole, no decision attached.

**End the quarter.** Every other capital moves, events fire, fronts shift,
markets reprice. You get an intelligence summary and the net change to your
position. Repeat twelve times.

### What a new player usually gets wrong

- **Spending everything early.** Political capital regenerates from approval
  and cohesion. Burn those and you spend the rest of the campaign unable to act.
- **Repeating a move that worked.** Every instrument has diminishing returns.
  The second identical sanctions package lands on an economy that has already
  re-routed around the first.
- **Sanctioning alone.** Bite scales with coalition breadth. Build the coalition
  first — it multiplies every economic instrument you use afterwards.
- **Ignoring red-line percentages.** They are shown before you commit, and
  crossing one hands the next move to someone else.
- **Treating military instruments as strong moves.** Most are net-negative
  under the model. Air defence, intelligence sharing and budget support usually
  outperform strikes, which is what the historical record suggests too.
- **Watching only the theatres.** Campaigns are lost at home more often than
  abroad: below 12 approval your government falls and the campaign ends there.
...
build/inline.js         single-file build
```
